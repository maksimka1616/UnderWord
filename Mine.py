...
def info():
    return (
        "Модуль для автоматического копания в @mine_evo_bot.\n\n"
        "Доступные команды:\n"
        "/minestart - запуск авто-копания\n"
        "/minestop - остановка авто-копания\n"
        "/minekd <секунды> - установка задержки перед копанием\n\n"
    )
...
async def set_mine_kd(event):
    if event.sender_id == owner:
        ...
        await event.edit(f"⚡️⛏️ КД авто-копания установлено на {kd_formatted} секунд ⛏️⚡️")

async def start_mining(event):
    if event.sender_id == owner:
        if get_mining_state():
            await event.edit("Авто-копание уже запущено.")
            return

        set_mining_state(True)
        await event.edit("⚡️⛏️ Авто-копание включено ⛏️⚡️")
        await auto_mine()
...
async def stop_mining(event):
    ...
    if event.sender_id == owner:
        if not get_mining_state():
            await event.edit("Авто-копание уже остановлено.")
            return

        set_mining_state(False)
        await event.edit("⚡️⛏️ Остановка авто-копания... ⛏️⚡️")
        ...
        await event.edit(f"⚡️⛏️ Копание выключено ⛏️⚡️\n\n"
